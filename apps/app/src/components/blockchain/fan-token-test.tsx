...
export function FanTokenTest() {
  ...
  const checkTokenBalances = async () => {
    ...
    try {
      ...
      console.log('checkTokenBalances', { address, fanTokens });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Test des Fan Tokens</CardTitle>
        <p className='text-sm text-muted-foreground'>Connecté avec: {address}</p>
      </CardHeader>
          ...
          <div className='space-y-3'>
            ...
            <div className='mt-4 p-3 bg-muted rounded-lg'>
              <p className='text-sm text-muted-foreground'>
                <strong>Note:</strong> Vous devez posséder au minimum 50 tokens du fan token requis pour participer à
                une raffle.
              </p>
            </div>
          </div>
          ...
          <div className='text-xs text-muted-foreground space-y-1'>
            {fanTokens.map((token) => (
              <div key={token.symbol} className='flex justify-between'>
                ...
                <code className='bg-muted px-1 rounded'>{token.address || 'Non configuré'}</code>
              </div>
            ))}
          </div>
        </div>
